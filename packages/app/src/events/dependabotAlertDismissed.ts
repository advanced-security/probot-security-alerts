 ...
 */
export async function dependabotAlertDismissed(
  context: Context<'dependabot_alert.dismissed'>
) {
  ...
  const alertSeverity = Math.max(
    toSeverity(
      context.payload.alert.security_advisory.severity,
      Severity.UNKNOWN
    ),
    toSeverity(
      context.payload.alert.security_vulnerability.severity,
      Severity.UNKNOWN
    )
  );
  ...
  if (alertSeverity < minimumSeverity) {
    context.log.info(
      `Alert close request allowed. Severity '${Severity[alertSeverity]}' is below minimum severity '${Severity[minimumSeverity]}'.`
    );
    return;
  }
  ...
  if (isMemberApproved) {
    ...
    await context.octokit.dependabot.updateAlert({
      owner,
      repo,
